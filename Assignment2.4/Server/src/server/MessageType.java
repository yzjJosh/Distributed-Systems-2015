...
enum MessageType {
	...
	RESERVE_SEATE,			//The message used for a client to request the server to reserve seats.
	SEARCH_SEATE,			//The message used for a client to request the server to search seats reserved by a name.
	DELETE_SEATE ,			//The message used for a client to request the server to release seats reserved by a name.
